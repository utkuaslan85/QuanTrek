...
import pandas as pd
import numpy as np
from bucket_v1 import process_orderbook, buckets
from helper import get_now

# Configure logging
# logging.basicConfig(level=logging.INFO)
logging.basicConfig(
    filename="/mnt/vol1/logs/orderbook_transform.log",  # Ensure this directory exists
    level=logging.INFO,
...
class NumpyPandasEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        if isinstance(obj, pd.DataFrame):
            return obj.to_dict(orient="list")  # JSON-friendly
...
class SimpleBinanceRecorder:
    ...
    def handle_depth_cache(self, depth_cache):
        ...
        try:
            ...
            df = pd.json_normalize(depth_data)
            # print(df.info())
            result = process_orderbook(df, buckets)
            # types_map = {k: type(v) for k, v in result.items()}
            # print(types_map)
            # Schedule async publish to JetStream
            if hasattr(self, 'loop') and self.loop and not self.loop.is_closed():
                asyncio.run_coroutine_threadsafe(
                    broker.publish(
                        json.dumps(result, cls=NumpyPandasEncoder),
                        # max_age=60*60*2, 
                        subject=subject, 
